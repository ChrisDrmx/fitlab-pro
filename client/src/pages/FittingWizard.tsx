...
import { blankClub, blankFitting, blankTrackman, Fitting, Insight, LieRow, TrackmanRow } from "../lib/types";

const steps = ["Joueur", "Transcription", "Mesures", "Matériel", "Lie board", "TrackMan", "Diagnostic & rapport"];
const goals = ["Plus de distance", "Plus de précision", "Trajectoire", "Confort", "Tolérance", "Gapping"];
...
export default function FittingWizard() {
  ...
  const reco = (patch: Partial<Fitting["reco"]>) => setData((previous) => ({ ...previous, reco: { ...previous.reco, ...patch } }));
  const updateClub = (index: number, patch: Partial<Fitting["currentClubs"][number]>) => update({ currentClubs: data.currentClubs.map((row, rowIndex) => rowIndex === index ? { ...row, ...patch } : row) });
  const updateLie = (index: number, patch: Partial<LieRow>) => update({ lieTests: data.lieTests.map((row, rowIndex) => rowIndex === index ? { ...row, ...patch } : row) });
  const updateTrackman = (index: number, patch: Partial<TrackmanRow>) => update({ trackman: data.trackman.map((row, rowIndex) => rowIndex === index ? deriveTrackman({ ...row, ...patch }) : row) });
